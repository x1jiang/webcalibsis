function [fig, targets_means, predictions_means,numb_bin] = NMC_plotreliability(targets, predictions, show_fig, titleName)
...
for bin = 1:10
    ...
    numb_bin(end+1)=length(predictions_in_bin);
    predictions_means(bin) = mean(predictions_in_bin);
    
    targets_in_bin = targets(bins == bin);
    targets_means(bin) = mean(targets_in_bin);
...
title(axes1, sprintf('%s:%s','Reliability Diagram',titleName))
